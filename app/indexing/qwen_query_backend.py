...
class QwenPersistentQueryEmbeddingBackend:
    ...
    def _status_ready(self, status: dict[str, Any]) -> bool:
        try:
            ...
            return (
                ...
                and status.get("generationId") == self.model_id
    ...
    def _ensure_worker(self) -> None:
        ...
        if status.get("state") in {"loading", "ready"} and status.get("generationId") != self.model_id:
            ...
            old_deadline = time.monotonic() + 15.0
            while time.monotonic() < old_deadline:
                if self._read_status().get("state") in {"stopped", "failed", None}:
                    break
                time.sleep(0.1)
    ...
    def _invoke(self, payload: dict[str, Any], *, expected_count: int) -> list[Vector]:
        ...
        request = {**payload, "generationId": self.model_id}
        ...
        if response.get("generationId") != self.model_id or int(response.get("dimension", 0)) != self.dimension:
            raise EmbeddingBackendError("Persistent Qwen query worker returned the wrong embedding generation")
