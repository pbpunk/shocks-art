...
function Stop-BridgeWorker {
    ...
    if (-not (Test-Path -LiteralPath $PidPath)) {
        Write-Host "$ProcessLabel is already stopped."
        return
    }
    $WorkerPid = 0
    ...
    if ($WorkerPid -le 0) {
        ...
        Write-Host "Removed invalid $ProcessLabel PID metadata."
        return
    }
    $Process = Get-CimInstance Win32_Process -Filter "ProcessId=$WorkerPid" -ErrorAction SilentlyContinue
    if (-not $Process) {
        Remove-Item -LiteralPath $PidPath -Force -ErrorAction SilentlyContinue
        Write-Host "$ProcessLabel was not running."
        return
    }
    if ($Process.CommandLine -notmatch [regex]::Escape($ScriptName)) {
        throw "Refusing to stop PID $WorkerPid because it is not the Shock's Art $ProcessLabel."
    }
    ...
    while ($Stopwatch.Elapsed.TotalSeconds -lt 10) {
        if (-not (Get-Process -Id $WorkerPid -ErrorAction SilentlyContinue)) {
            Remove-Item -LiteralPath $PidPath -Force -ErrorAction SilentlyContinue
            Write-Host "$ProcessLabel stopped."
            return
        }
...
Stop-BridgeWorker -ScriptName "google_update_worker.py" -ProcessLabel "Google update worker" -PidFile "google_update_worker.pid"
