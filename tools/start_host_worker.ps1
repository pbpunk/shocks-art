...
if (-not $env:SHOCKS_GOOGLE_SPREADSHEET_ID) {
    Write-Host "Shock's Art GPT Bridge is not configured; host worker remains disabled."
    exit 0
}

$PidPath = Join-Path $DataDir "google_host_worker.pid"
$StatusPath = Join-Path $DataDir "google_host_worker_status.json"
$OutLog = Join-Path $LogsDir "google_host_worker.out.log"
$ErrLog = Join-Path $LogsDir "google_host_worker.err.log"
...
if (Test-Path -LiteralPath $PidPath) {
    ...
    if ($ExistingPid -gt 0 -and (Get-Process -Id $ExistingPid -ErrorAction SilentlyContinue)) {
        Write-Host "Google host worker is already running (PID $ExistingPid)."
        exit 0
    }
...
$Args = @($Python.PrefixArgs) + @("tools/google_host_worker.py")
$Worker = Start-Process -FilePath $Python.FilePath `
    -ArgumentList $Args `
...
if (-not (Get-Process -Id $Worker.Id -ErrorAction SilentlyContinue)) {
    Remove-Item -LiteralPath $PidPath -Force -ErrorAction SilentlyContinue
    throw "Google host worker exited during startup. Inspect $ErrLog"
}
