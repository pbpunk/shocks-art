...
JOURNAL_PATH = Path(
    os.getenv("SHOCKS_GOOGLE_HOST_JOURNAL_PATH", str(ROOT / "data" / "google_host_journal.json"))
)
STATUS_PATH = Path(
    os.getenv("SHOCKS_GOOGLE_HOST_STATUS_PATH", str(ROOT / "data" / "google_host_worker_status.json"))
)
WORKTREE_ROOT = Path(
    os.getenv(
        "SHOCKS_GOOGLE_HOST_WORKTREE_ROOT",
        str(Path(tempfile.gettempdir()) / "shocks-art-host-worktrees"),
    )
)
TIMEOUT_SECONDS = max(60, int(os.getenv("SHOCKS_GOOGLE_HOST_TIMEOUT_SECONDS", "14400")))
RESULT_LIMIT = 30000
stopping = False
...
def run(args: list[str], *, cwd: Path = ROOT, timeout: int = 120) -> subprocess.CompletedProcess[str]:
...
def refresh_refs() -> str:
    ...
    git(
        "fetch",
        "origin",
        "+refs/heads/autonomous/*:refs/remotes/origin/autonomous/*",
        "--prune",
        timeout=120,
    )
...
def autonomous_tips() -> dict[str, str]:
    output = git(
        ...
        "refs/remotes/origin/autonomous/",
        timeout=10,
    )
    tips: dict[str, str] = {}
...
def revision_allowed(expected: str) -> tuple[bool, str]:
    main = refresh_refs()
    if expected == main:
        return True, "origin/main"
    matching = [ref for ref, sha in autonomous_tips().items() if sha == expected]
    if not matching:
        return False, "revision is neither origin/main nor the tip of origin/autonomous/*"
...
def safe_worktree(request_id: str) -> Path:
    ...
    target = (WORKTREE_ROOT / request_id).resolve()
...
def create_worktree(request_id: str, revision: str) -> Path:
    ...
    if local_revision(target) != revision:
        remove_worktree(target)
        raise RuntimeError("detached worktree revision mismatch")
    if tracked_dirty(target):
        remove_worktree(target)
        raise RuntimeError("detached worktree is unexpectedly dirty")
    return target
...
def write_state(service, status: str, *, last_request: str = "", error: str = "", active: str = "") -> None:
    ...
    try:
        ...
        pass
    update_values(service, f"{STATE_SHEET_NAME}!A2:F2", [[now_iso(), status, revision, last_request, error, active]])
    atomic_json(
        STATUS_PATH,
        {
            "schemaVersion": SCHEMA_VERSION,
            "process": "google_host_worker",
            "pid": os.getpid(),
            "status": status,
            "heartbeatTimestamp": now_iso(),
            "revision": revision,
            "lastRequestId": last_request,
            "lastError": error,
...
def execute_request(service, row_number: int, request, journal: dict[str, Any]) -> None:
    ...
    existing = journal["requests"].get(request.request_id)
    ...
    allowed, ref_name = revision_allowed(request.expected_revision)
    if not allowed:
        row = result_row(request, state="rejected", finished=now_iso(), outcome="REJECT", summary=ref_name)
        journal["requests"][request.request_id] = {"fingerprint": fingerprint, "state": "rejected", "row": row}
        atomic_json(JOURNAL_PATH, journal)
        write_row(service, row_number, row)
        return

    started = now_iso()
    write_row(service, row_number, result_row(request, state="running", started=started))
    ...
    try:
        ...
        result = subprocess.run(
            [sys.executable, "tools/run_host_profile.py", request.profile],
            cwd=target,
            env=env,
            text=True,
            capture_output=True,
        ...
        # Re-fetch after the run so a moving autonomous branch cannot lend a stale PASS.
        refresh_refs()
        if request.expected_revision != git("rev-parse", "origin/main", timeout=10).lower():
            if request.expected_revision not in autonomous_tips().values():
                raise RuntimeError("candidate autonomous branch moved during verification")
        ...
        row = result_row(
            ...
            duration=duration,
            summary=summary,
            result_json=json.dumps(parsed, separators=(",", ":"), sort_keys=True),
        )
    except Exception as exc:
        finished = now_iso()
        duration = round(time.monotonic() - started_clock, 3)
        row = result_row(request, state="failed", started=started, finished=finished, tested=request.expected_revision, outcome="FAIL", exit_code=1, duration=duration, summary=str(exc))
    finally:
        cleanup_ok = True if target is None else remove_worktree(target)
        if not cleanup_ok:
            row[5] = "failed"
            row[9] = "FAIL"
            row[12] = (row[12] + "; detached worktree cleanup failed")[:1500]

    journal["requests"][request.request_id] = {"fingerprint": fingerprint, "state": row[5], "row": row}
    atomic_json(JOURNAL_PATH, journal)
    write_row(service, row_number, row)
...
def process_once(service, journal: dict[str, Any]) -> None:
    for row_number, raw in read_rows(service):
        ...
        try:
            ...
            # Malformed request IDs cannot safely key the journal, but the row can still be rejected in place.
            row[5] = "rejected"
            row[7] = now_iso()
            row[9] = "REJECT"
            row[12] = str(exc)[:1500]
...
def main() -> int:
    ...
    while not stopping:
        try:
            ...
            write_state(service, "error", error=str(exc))
