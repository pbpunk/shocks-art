...
def main() -> int:
    ...
    for model_id, compute_type in CANDIDATES:
        ...
        try:
            ...
            case_results = []
            for case in cases:
                media_value = case.get("media_path", case.get("path", ""))
                media = Path(str(media_value))
                if not media.is_file():
                    raise RuntimeError(f"benchmark media missing: {media.name}")
                started = time.monotonic()
                segments, info = model.transcribe(str(media), language=case.get("language") or "en", word_timestamps=True, vad_filter=True)
                ...
                terms = case.get("project_terms", case.get("expected_terms", []))
                normalized_text = normalized_phrase(transcript)
                normalized_terms = [normalized_phrase(str(term)) for term in terms if str(term).strip()]
                ...
                case_results.append({"case_id": str(case.get("id") or media.name), "duration_seconds": round(duration, 3), "runtime_seconds": round(elapsed, 3), "general_accuracy": round(case_accuracy, 4), "project_term_hits": hits, "project_term_total": len(normalized_terms), "timestamp_valid": valid, "timestamp_total": len(materialized)})
            row = {
